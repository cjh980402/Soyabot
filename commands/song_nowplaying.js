...
module.exports = {
    ...
    async execute(message) {
        ...
        if (song.duration >= 0) {
            nowPlaying.addField('\u200b', `${new Date(seek * 1000).toISOString().substr(11, 8)} [${splitBar(song.duration || seek, seek, 20)[0]}] ${song.duration == 0 ? '◉ LIVE' : new Date(song.duration * 1000).toISOString().substr(11, 8)}`, false);
        }
        if (song.duration > 0) {
