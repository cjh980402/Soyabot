...
- ${client.prefix}빵: 본인의 차례를 수행합니다.`,
    ...
    async execute(message, args) {
        ...
        for (let i = 0; i < bullet; i++) {
            ...
            if (i === die) {
                try {
                    const dieUser = await message.guild.members.fetch(gameUser[i % gameUser.length].id, { cache: false });
                ...
                try {
                    const nextUser = await message.guild.members.fetch(gameUser[(i + 1) % gameUser.length].id, { cache: false });
