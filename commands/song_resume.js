...
export const usage = `${client.prefix}resume`;
export const command = ['resume', 'rs'];
export const description = '- 가장 최근 노래를 다시 재생합니다.';
...
export async function commandExecute(interaction) {
    ...
    if (!queue.playing) {
        queue.player.unpause();
        return interaction.followUp(`${interaction.user} ▶ 노래를 다시 틀었습니다.`);
    }

    return interaction.followUp('대기열이 일시정지 상태가 아닙니다.');
}
