...
module.exports = {
    ...
    async execute(message) {
        ...
        const sunday = eventdata.find('dt a').filter((_, v) => /^썬데이\s*메이플$/.test($(v).text())).attr('href');
        if (!sunday) {
            return message.channel.send('썬데이 메이플 공지가 아직 없습니다.');
        }

        const sundayTitle = `${eventdata.find('dd a').filter((_, v) => $(v).attr('href') === sunday).text().substr(0, 14)}의 썬데이 메이플`;
