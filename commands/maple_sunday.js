...
module.exports = {
    ...
    async execute(message) {
        ...
        const sunday = eventdata.find("li dl dt a").filter((i, v) => /^썬데이\s*메이플$/.test(parse(v).text())).attr("href");
        if (!sunday) {
            return chat.reply('썬데이 메이플 공지가 아직 없습니다.');
        }

        const sundayTitle = `${eventdata.find("li dl dd a").filter((i, v) => parse(v).attr("href") == sunday).text().substr(0, 14)}의 썬데이 메이플`;
