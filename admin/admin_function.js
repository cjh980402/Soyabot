...
module.exports.adminChat = async function (message) {
    ...
    if (fullContent.startsWith('>')) {
        ...
        message.channel.send({ content: String(await eval(`(async () => {\n${funcBody.join('\n')}\n})()`)) || '\u200b', code: 'js', split: { char: '' } });
        // eval의 내부가 async 함수의 리턴값이므로 await까지 해준다. js의 코드 스타일을 적용해서 출력한다.
    } else if (fullContent.startsWith(')')) {
        // 콘솔 명령 실행 후 출력
        message.channel.send({ content: (await module.exports.cmd(fullContent.substr(1).trim(), true)) || '\u200b', code: 'shell', split: { char: '' } });
    } else if (room) {
